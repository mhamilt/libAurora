                                      ...
                                      const bool replaceDotsWithUnderscores)
{
    ...
    char buffer[32];
    std::snprintf(buffer, sizeof(buffer), (frac > 0.0f ? "%.1f" : "%.0f"), f);
...
std::string Aurora::GetBandLabel(const float fcb,
                                 const bool replaceDotsWithUnderscores)
{
    assert(fcb > 0.0f && fcb <= 20000.0f);
...
std::string Aurora::ModuleVersionNumber(const int nType, const int nRc)
{
    std::tm tm = {};
    ...
    version << AURORA_VERSION_String;// << "." << AURORA_BUILD_COUNT;
...
std::string Aurora::ModuleVersionString()
{
